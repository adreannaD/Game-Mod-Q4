...
void rvWeaponRocketLauncher::Spawn ( void ) {
	...
	attackDict.GetFloat ( "speed", "9.99", guideSpeedFast );
	guideSpeedSlow = guideSpeedFast * f;
	
	reloadRate = SEC2MS ( spawnArgs.GetFloat ( "reloadRate", "9.99" ) );
	
	guideAccelTime = SEC2MS ( spawnArgs.GetFloat ( "lockAccelTime", "9.99" ) );
	...
	if ( animNum ) {
		...
		rate = (float)anim->Length() / (float)SEC2MS(spawnArgs.GetFloat ( "reloadRate", "9.99" ));
